...
### 10.4 Prefixo personalizável por utilizador + Activities + gravações (2026-07-07)

Três pedidos do utilizador na mesma sessão:

**Prefixo de saída agora também por utilizador** (mesmo padrão da extensão): `zadarma_user_extensions` ganhou uma coluna `outbound_prefix` (nullable, `extension` também passou a nullable já que os dois campos agora são independentes). `CallController::store()` resolve `$userExtension?->outbound_prefix ?? config('zadarma.outbound_prefix')`, mesma lógica de reserva já usada para a extensão. Editável no mesmo mini-componente `v-zadarma-my-extension` da página "My Account".

**Chamadas registadas como Activity (`type=call`)**: `CallRecordSync::logActivity()` cria uma `Webkul\Activity\Models\Activity` (`type: 'call'`, `schedule_from`/`schedule_to` calculados a partir de `started_at`+`duration`, `is_done: true`, `user_id` do utilizador atribuído) ligada via `person_activities` — **só à Pessoa, nunca a um Lead** (decisão explícita do utilizador: `call_records.person_id` é a única associação fiável que os dados têm; uma pessoa pode ter vários leads e não há como saber qual). Idempotente via nova coluna `call_records.activity_id`: reprocessar a mesma chamada (webhook repetido, sobreposição de janela de polling) atualiza a Activity existente em vez de duplicar.
- **Armadilha encontrada a testar isto**: a trait `Webkul\Activity\Traits\LogsActivity` (usada pelo `Person`) cria **automaticamente** uma Activity `type: 'system'` sempre que uma Pessoa é criada/atualizada — isto é comportamento nativo do Krayin, não relacionado com o nosso código. Um teste inicial contou `$person->activities()->count()` esperando 1 e viu 2, o que pareceu um bug de duplicação — na realidade eram duas activities de tipos diferentes (`system` do Krayin + `call` nossa). Filtrar por `type='call'` confirma que a nossa lógica de idempotência está correta. **Lição: ao verificar contagens de relações em modelos Krayin, confirmar se o modelo usa `LogsActivity` antes de assumir que só a nossa escrita está a contribuir para a contagem.**

**Endpoint de gravações ligado**: `ZadarmaClient::getRecordingLink(callId)` chama `GET /v1/pbx/record/request/` (endpoint confirmado na Fase 3, nunca antes usado) e devolve `link` (ou o primeiro de `links`), ou `null` em qualquer falha (nunca lança exceção — não ter gravação é um resultado normal, não um erro). Chamado em dois pontos:
- `SyncCallsCommand`: depois de cada `upsert()`, se `duration > 0` e ainda não há `recording_url` (chamadas com duração 0 nunca têm gravação, poupa-se o pedido extra à API).
- `WebhookController::handleRecordReady()` (evento `NOTIFY_RECORD`, novo): mais correto do que tentar à chegada do `NOTIFY_END`, porque a gravação provavelmente ainda não está pronta nesse momento.
- **Não validado com uma gravação real** (só testado estruturalmente com um `call_id` inexistente, confirmando que falha graciosamente sem *crash*) — a Fase 7 só tinha chamadas sem gravação disponível para testar.
- O link devolvido pela Zadarma é temporário (`lifetime_till`, visto na Fase 3) — guardamos o valor uma única vez em `call_records.recording_url` e nunca o renovamos; se expirar, o leitor de áudio simplesmente falha a reproduzir. Não implementado renovação automática (fora do âmbito pedido).

**Leitor de áudio**: adicionado `<audio controls preload="none">` em `call-history.blade.php` quando `recording_url` existe, **só na nossa secção "Call History"** — decisão explícita do utilizador para não mexer no separador nativo "Activities" do Krayin (que não tem nenhum precedente de leitor de áudio).

### 10.5 Bug crítico de fuso horário encontrado e corrigido (2026-07-07)

O utilizador reportou que uma chamada real feita "agora" não ficou registada em lado nenhum. Investigação revelou dois problemas distintos:

1. **Bug real e crítico, agora corrigido**: `SyncCallsCommand` usava `now()` diretamente para calcular a janela `start`/`end` enviada à Zadarma. Esta instalação Krayin tem `APP_TIMEZONE=Asia/Kolkata` (valor por omissão do instalador, nunca alterado neste ambiente de referência) — mas a Zadarma trata os timestamps como UTC. Isto desalinhava a janela em ~5h30 face ao tempo real. Na Fase 7, a **primeira sincronização** (janela de 24h, sem `last_synced_at` anterior) ainda "funcionou" por sorte — uma janela suficientemente larga sobrepõe-se ao período certo mesmo desalinhada. Mas como `last_synced_at` avança a cada sincronização bem-sucedida, as janelas seguintes ficaram cada vez mais estreitas (~10 minutos, o intervalo do agendamento) — e uma janela estreita desalinhada em 5h30 **nunca** apanha uma chamada real. **Corrigido**: `SyncCallsCommand` e `WebhookController` agora tratam explicitamente `now()`/`callstart`/`call_start` como UTC (`now()->utc()`, `Carbon::parse(..., 'UTC')->setTimezone(config('app.timezone'))`), independentemente do fuso horário configurado na instalação Krayin. Validado: depois da correção, uma sincronização manual apanhou uma chamada real de ontem (15:53 UTC) que a versão anterior nunca teria encontrado com uma janela estreita.
2. **Achado separado, não é bug**: **não existe nenhum `cron` a correr dentro do container `app`** desta instância de referência (confirmado: `crontab: not found`) — o agendamento `Schedule::command(...)->everyTenMinutes()` só existe no código, mas nada invoca `php artisan schedule:run` periodicamente neste ambiente Docker de desenvolvimento. Todas as sincronizações desta sessão até agora foram manuais. Isto é esperado para um ambiente de referência/testes (não faz parte do package a distribuir), mas **numa instalação real, o dono do servidor tem de configurar isto** (crontab do sistema com `* * * * * php artisan schedule:run`, ou um mecanismo equivalente — prática standard do Laravel, não algo que o nosso package possa forçar).

**Atualização (resolvido no mesmo dia): a causa real era outra, mais fundamental.** Mesmo depois da correção de fuso horário, a chamada específica que o utilizador reportou continuava a não aparecer em `/v1/statistics/`, nem numa janela alargada de 24h. O utilizador reparou que no painel da Zadarma **"A minha conta > Estatística de chamadas" não mostra chamadas recentes, mas "Minha Central Telefónica > Estatísticas e gráficos" mostra todas** — pista decisiva. Confirmado por documentação e testado com sucesso contra a conta real: `/v1/statistics/` e `/v1/statistics/pbx/` são **dois relatórios diferentes da Zadarma**, e chamadas encaminhadas pela Central/extensões (exatamente o que o nosso botão de ligar e o uso normal do telefone da equipa produzem) só aparecem no segundo.

### 10.6 Correção final (2026-07-07): endpoint errado usado desde a Fase 3

`SyncCallsCommand` mudou de `/v1/statistics/` para `/v1/statistics/pbx/` — **confirmado o endpoint certo para chamadas de central/extensão**, com bónus reais:
- **Campos confirmados via conta real**: `call_id`, `sip`, `callstart`, `clid` (número de origem), `destination` (número de destino), `disposition`, `seconds` (duração), `is_recorded` (boolean), `pbx_call_id`.
- **Direção da chamada finalmente resolvida** (limitação conhecida desde a Fase 3): `call_type` (`in`/`out`) é um **filtro** do pedido, não um campo devolvido — por isso o comando agora faz **duas chamadas por sincronização** (uma com `call_type=in`, outra com `call_type=out`), etiquetando a direção com confiança em vez de cair sempre em `'unknown'`.
- **`is_recorded`** permite saber antecipadamente se vale a pena chamar `/v1/pbx/record/request/` (antes tentava sempre que `duration > 0`, agora só quando `is_recorded` é verdadeiro — menos pedidos desnecessários à API).
- **Validado com sucesso de ponta-a-ponta contra a chamada real do utilizador**: direção `outbound` correta, `person_id` e `user_id` corretamente atribuídos, `Activity` criada e visível no separador nativo "Activities" do Krayin (`"Outbound call with ... — duration 00:00:07, disposition: answered"`), e **`recording_url` populado automaticamente com um link real** — confirmado visualmente no browser com o leitor `<audio>` a aparecer preenchido.
- **Nota de performance**: a documentação oficial da Zadarma desaconselha sondar este endpoint a cada minuto — recomenda usar webhooks para eventos em tempo real. O nosso intervalo de 10 em 10 minutos está dentro do razoável, mas reforça que o modo `webhook` é a via preferida para instalações que o consigam usar.

### 10.7 Limitação encontrada (2026-07-20): extensão configurada tarde não atribui chamadas antigas

Reportado pelo utilizador: um utilizador (extensão pessoal configurada só recentemente) não via as suas chamadas na página de Estatísticas, apesar de ter feito chamadas reais.

- **Não era um bug de correspondência** — `CallRecordSync::matchUser($sip)` funciona corretamente (confirmado diretamente contra a extensão real). O problema é a limitação já conhecida da secção 10 ("chamadas antigas ficam com `user_id = null`, não há reprocessamento retroativo"): a extensão do utilizador só foi guardada em `zadarma_user_extensions` depois de ele já ter feito centenas de chamadas, e a sincronização só resolve `user_id` no momento em que a chamada é inserida pela primeira vez.
- **Confirmado ao vivo**: o campo `sip` devolvido por `/v1/statistics/pbx/` corresponde exatamente ao valor de extensão guardado (ex. `104`), tal como esperado — a única lacuna era temporal, não de lógica.
- **Corrigido com um novo comando reutilizável** `zadarma:backfill-users` (`Console/Commands/BackfillCallUsersCommand.php`, registado no `ZadarmaServiceProvider`): volta a pedir `/v1/statistics/pbx/` (in+out) para cobrir todo o intervalo de `call_records` com `user_id` em falta, constrói um mapa `call_id → sip`, e atualiza apenas os registos ainda não atribuídos — idempotente, não duplica nem sobrescreve atribuições já corretas. Corrido manualmente uma vez para corrigir o histórico existente; não está agendado (é uma correção pontual, não uma tarefa periódica), mas pode ser corrido de novo sempre que este cenário se repetir (nova pessoa a configurar a extensão depois de já ter chamadas antigas).
