...
- **`WebhookSignatureVerifier` corrigido pelo mesmo motivo mas continua sem validação contra uma notificação real** (precisa de um túnel público tipo `ngrok` para receber uma notificação real da Zadarma — ainda não configurado). O `CallController`/`/v1/request/callback/` também usa o `ZadarmaClient` já corrigido mas **não foi testado com uma chamada real** (colocar uma chamada real desliga um telefone verdadeiro — só deve ser feito com autorização explícita do utilizador).
