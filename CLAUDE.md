...
- **Ambiente de desenvolvimento montado**: `docker-compose.yml` + `docker/php/Dockerfile` (PHP 8.3-FPM com `pdo_mysql, mbstring, exif, pcntl, bcmath, gd, zip, intl, xml, calendar`) + `docker/nginx/default.conf`, na raiz deste repo. Krayin clonado em `krayin-app/` (fora do controlo de versão deste projeto — é só instância de referência/desenvolvimento, não faz parte do package a distribuir). Portas: app web em `:8080` (evita conflito com o OpenCRM, que usa `:5432`/`:9000-9001`), MySQL em `:3306`.
