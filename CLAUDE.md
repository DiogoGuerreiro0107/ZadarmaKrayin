...
- **Por confirmar com conta real (Fase 7) — não assumir como certo:**
  ...
  - **Chave do envelope da resposta**: assumi `$response['stats']` para a lista de chamadas (comum em várias APIs deste estilo), mas não foi confirmado contra uma resposta real da Zadarma — verificar no primeiro teste com credenciais reais e ajustar em `SyncCallsCommand::handle()` se necessário.
