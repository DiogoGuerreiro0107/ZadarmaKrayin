...
**Ainda por confirmar**: mesmo depois da correção, a chamada específica que o utilizador reportou como "feita agora" continuava a não aparecer numa janela UTC alargada (últimas 24h) — só apareceram duas chamadas mais antigas (uma já conhecida da Fase 7, outra de ontem que nunca tinha sido sincronizada por causa do bug acima). Hipóteses por explorar: atraso de processamento da Zadarma antes de uma chamada aparecer em `/v1/statistics/`, ou o número chamado não corresponder a nenhuma Person na CRM (nesse caso `person_id` fica `null` e a chamada não aparece em nenhuma ficha, mesmo estando corretamente gravada em `call_records`).
